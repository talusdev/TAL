...
TAL now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<tal_address\> \<monthly_payment_tal\> [use_ix(true|false)]
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ TAL, _12_ times over the course of a year totaling _24000_ TAL.
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<tal_address\> \<monthly_payment_tal\> \<collateral_hash\>
...
When block `1000000` is reached you'll receive a payment for `1200` TAL.
 ...
 * vote-many          - Vote on a TAL initiative
 * vote-alias         - Vote on a TAL initiative
 * vote               - Vote on a TAL initiative/budget
