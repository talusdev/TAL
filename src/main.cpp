...
#if defined(NDEBUG)
#error "TAL cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "TALUS Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
	...
	if(nHeight <= 1 && nHeight > 0) {
		...
		nSubsidy = 90230 * COIN; 
	} else if (nHeight > 3 && nHeight <= 500) {
		nSubsidy = 1 * COIN;
	} else if (nHeight > 500 && nHeight <= 2100000) {
		nSubsidy = 10 * COIN;
	}
    return nSubsidy;
}
